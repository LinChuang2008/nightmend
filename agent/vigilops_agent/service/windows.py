...
用法（需管理员权限）：
    vigilops-agent service install   # 安装服务
    vigilops-agent service start     # 启动服务
    vigilops-agent service stop      # 停止服务
    vigilops-agent service restart   # 重启服务
    vigilops-agent service remove    # 卸载服务
    vigilops-agent service status    # 查看状态
"""
import logging
import subprocess
import sys
...
SERVICE_NAME = "VigilOpsAgent"
SERVICE_DISPLAY_NAME = "VigilOps Monitoring Agent"
SERVICE_DESCRIPTION = "VigilOps 轻量级监控代理，负责采集系统指标、服务健康检查和日志上报。"
...
def _get_service_class():
    ...
    class VigilOpsAgentService(win32serviceutil.ServiceFramework):
        ...
        def SvcDoRun(self):
            ...
            try:
                ...
                servicemanager.LogErrorMsg(f"VigilOps Agent service error: {e}")
        ...
        def _run_agent(self):
            ...
            try:
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,
                                     r"SYSTEM\CurrentControlSet\Services\VigilOpsAgent\Parameters")
            ...
            if not config_path or not Path(config_path).exists():
                candidates = [
                    Path("C:/ProgramData/VigilOps/config.yaml"),
                    Path("/etc/vigilops/config.yaml"),
                ]
                config_path = next((str(p) for p in candidates if p.exists()), None)

            if not config_path:
                raise FileNotFoundError(
                    "No config file found. Run 'vigilops-agent configure' first."
                )
...
def install_service() -> bool:
    ...
    try:
        ...
        print(f"   运行 'vigilops-agent service start' 启动服务。")
        return True
    except Exception as e:
        print(f"❌ 安装失败: {e}")
        return False
...
def _save_config_path_to_registry():
    ...
    try:
        import winreg
        config_path = str(Path.home() / ".vigilops" / "config.yaml")
