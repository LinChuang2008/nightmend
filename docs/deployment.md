...
| 端口 | 3002 (前端) · 8001 (后端) · 5433 (pg) · 6380 (redis) · 8003 (mcp) | 同左 + 443 反代 |
...
#   FRONTEND_PORT=3002
#   PUBLIC_URL=http://your-host:3002
```
...
> 端口格式为「宿主端口 → 容器端口」；宿主端口由 `.env` 对应变量控制，下表为 `docker-compose.yml` 默认值。除后端、前端外，所有服务仅绑定 `127.0.0.1`（不对外暴露）。ClickHouse / Loki 受 compose profile 控制，需 `--profile clickhouse` / `--profile loki` 才启动。

| 容器 | 端口（宿主 → 容器） | 变量 | 作用 |
| --- | --- | --- | --- |
| `nightmend-postgres-1` | 5433 → 5432 | `POSTGRES_PORT` | 元数据存储 |
| `nightmend-redis-1` | 6380 → 6379 | `REDIS_PORT` | 缓存 + 队列 |
| `nightmend-clickhouse-1` | 8123/9000（同端口） | `CLICKHOUSE_HTTP_PORT` / `CLICKHOUSE_TCP_PORT` | 指标 OLAP（profile: clickhouse） |
| `nightmend-loki-1` | 3100（同端口） | `LOKI_PORT` | 日志存储（profile: loki） |
| `nightmend-backend-1` | 8001 → 8000 | `BACKEND_PORT` | FastAPI 主业务 |
| `nightmend-mcp-1` | 8003 → 8003 | `MCP_PORT` | Model Context Protocol server |
| `nightmend-frontend-1` | 3002 → 80 | `FRONTEND_PORT` | Nginx + React 静态资源 |
...
或自备 nginx / traefik，参考 `docs/integrations/https.md`。
...
AI 分析为可选能力，不配置不影响核心监控。启用步骤：

1. 在 https://platform.deepseek.com 申请 API Key
2. 安装时填入，或后续编辑 `.env`：

```env
AI_API_KEY=sk-your-key-here          # DeepSeek API Key，留空则关闭 AI 功能
AI_API_BASE=https://api.deepseek.com/v1   # AI 接口地址
AI_MODEL=deepseek-chat               # AI 模型名
```

3. 重启后端生效：`docker compose restart backend`
...
仅开放 443 + 22；内部端口用 Docker 默认 bridge 隔离。Postgres / Redis / ClickHouse / Loki 默认仅绑定 `127.0.0.1`，无需对外开放。
...
| 端口被占用 | 宿主端口冲突 | 改 `.env` 对应端口变量（`FRONTEND_PORT` / `BACKEND_PORT` / `POSTGRES_PORT` / `REDIS_PORT` 等）后 `docker compose up -d` |
| 忘记管理员密码 | — | 见下方「重置管理员密码」 |

### 重置管理员密码

通过容器内 `psql` 直接改写密码哈希（bcrypt）：
...
> 哈希值需用 bcrypt 预先生成；上例为占位。改后用新密码登录并在界面再次修改。
...
详见 `docs/internal/test-report-offline-deploy.md`。
...
- 《Agent 安装手册》`docs/agent-install.md`
- 《HTTPS 配置》`docs/integrations/https.md`
